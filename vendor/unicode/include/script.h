...
/**
 * @brief Returns the broad regional family of a script.
 *
 * See `decoder_script_family_t` for the (informal) taxonomy.
 *
 * @param script  A decoder_script_t value.
 * @return The family enum; DECODER_SCRIPT_FAMILY_UNKNOWN if unrecognised.
 */
decoder_script_family_t decoder_get_script_family(decoder_script_t script);

/**
 * @brief Returns the script-family name for a code point.
 *
 * Convenience wrapper that resolves the script for `cp` and returns the
 * family name as a static string ("African", "European", etc.).
 *
 * @param cp  Unicode code point.
 * @return Static null-terminated family name; never NULL.
 */
 ...
 *
 *  A mutable, in-memory tool for managing user-supplied script ranges with
 *  overlap detection and priority-based conflict resolution. Useful for
 *  diagnostics, alternate naming experiments, and merging UCD data with
 *  legacy or domain-specific mappings.
 * ═══════════════════════════════════════════════════════════════════════════ */

/**
 * @brief Initialises a script unification engine.
 *
 * @param engine        Engine to initialise (caller-allocated).
 * @param mode          Conflict-resolution policy.
 * @param strict_mode   Reserved flag stored on the engine; today does not
 *                      affect behaviour but is preserved for callers.
 * @return true on success.
 */
bool decoder_script_unification_init(decoder_script_unification_t *engine,
                                     decoder_script_resolution_mode_t mode,
                                     bool strict_mode);

/**
 * @brief Releases memory owned by the engine. Safe to call on a zeroed struct.
 */
void decoder_script_unification_cleanup(decoder_script_unification_t *engine);

/**
 * @brief Adds a script range to the engine.
 *
 * Range labels and source strings are copied (truncated at
 * `DECODER_SCRIPT_LABEL_MAX - 1` bytes). Overlapping ranges are accepted; the
 * resolver picks a winner by priority.
 *
 * @return true on success, false on allocation failure or invalid arguments.
 */
bool decoder_script_unification_add_range(decoder_script_unification_t *engine,
...
/**
 * @brief Detects overlapping ranges and reports the resolved (winning) script.
 *
 * One conflict result is emitted per overlapping pair. The `resolved_script`
 * field carries the higher-priority entry's label, with primary priority and
 * UCD-authoritative status as tie-breakers.
 *
 * @param conflicts        Output buffer; truncated to `max_conflicts`.
 * @param max_conflicts    Capacity of `conflicts`.
 * @param conflict_count   Out: number of conflicts written.
 * @return true on success.
 */
bool decoder_script_unification_resolve_conflicts(decoder_script_unification_t *engine,
                                                  decoder_script_conflict_result_t *conflicts,
                                                  size_t max_conflicts,
                                                  size_t *conflict_count);

/**
 * @brief Returns the resolved script label for a code point.
 *
 * Walks the engine's ranges and returns the label of the highest-priority
 * range covering `cp`. Returns NULL when no range covers it.
 */
const char *decoder_script_unification_get_script(const decoder_script_unification_t *engine,
                                                  uint32_t cp);

/**
 * @brief Reports gaps between configured ranges within `[start, end]`.
 *
 * Adjacent or overlapping input ranges are merged before gaps are emitted.
 *
 * @param gaps        Output buffer.
 * @param max_gaps    Capacity of `gaps`.
 * @param gap_count   Out: number of gaps written.
 * @return true on success.
 */
bool decoder_script_unification_find_gaps(const decoder_script_unification_t *engine,
                                          uint32_t start, uint32_t end,
                                          decoder_script_range_t *gaps, size_t max_gaps,
                                          size_t *gap_count);
