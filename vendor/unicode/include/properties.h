...
/**
 * @brief Returns the Indic_Syllabic_Category for a code point.
 *
 * Identifies a code point's role in Indic-syllable structure (vowel,
 * consonant, virama, etc.). Code points in non-Indic scripts return
 * `DECODER_INDIC_SYLLABIC_OTHER`.
 *
 * @param cp  Unicode code point.
...
/**
 * @brief Returns the Indic_Positional_Category for a code point.
 *
 * Describes where a dependent vowel or mark renders relative to its base
 * consonant (Top, Bottom, Left, etc.). Returns `DECODER_INDIC_POSITIONAL_NA`
 * when the property does not apply.
 *
 * @param cp  Unicode code point.
 * @return The positional category enum.
 */
decoder_indic_positional_category_t decoder_get_indic_positional_category(uint32_t cp);

/**
 * @brief Returns the human-readable name of an Indic syllabic category.
 *
 * @param category  An Indic syllabic category enum value.
 * @return Static null-terminated string (e.g. "Vowel_Independent"); never NULL.
 */
const char *decoder_get_indic_syllabic_category_name(decoder_indic_syllabic_category_t category);

/**
 * @brief Returns the human-readable name of an Indic positional category.
 *
 * @param category  An Indic positional category enum value.
 * @return Static null-terminated string (e.g. "Top_And_Bottom"); never NULL.
 */
