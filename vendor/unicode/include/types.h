...
/**
 * @brief Broad regional/cultural grouping of writing systems.
 *
 * The taxonomy is informal: Unicode does not define "script families."
 * Choices were made on cultural-region grounds (e.g. Cherokee → American,
 * N'Ko → African) and may be debatable for scripts at regional boundaries.
 * `COMMON` covers shared characters (digits, punctuation) and inherited
 * combining marks. `UNKNOWN` covers scripts the library has not classified.
 */
typedef enum {
    DECODER_SCRIPT_FAMILY_UNKNOWN = 0,
    DECODER_SCRIPT_FAMILY_COMMON,           /**< Common / Inherited. */
    DECODER_SCRIPT_FAMILY_AFRICAN,
    DECODER_SCRIPT_FAMILY_AMERICAN,
    DECODER_SCRIPT_FAMILY_EAST_ASIAN,
    DECODER_SCRIPT_FAMILY_EUROPEAN,
    DECODER_SCRIPT_FAMILY_MIDDLE_EASTERN,
    DECODER_SCRIPT_FAMILY_SOUTH_ASIAN,
    DECODER_SCRIPT_FAMILY_SOUTHEAST_ASIAN,
    DECODER_SCRIPT_FAMILY_SYMBOLS,          /**< Braille, SignWriting, shorthand. */
    DECODER_SCRIPT_FAMILY_COUNT
} decoder_script_family_t;

/**
 * @brief Indic_Syllabic_Category property (UAX #44).
 *
 * Sourced from `IndicSyllabicCategory.txt`. Code points outside any Indic
 * script (or unassigned) report `DECODER_INDIC_SYLLABIC_OTHER`.
 */
typedef enum {
    DECODER_INDIC_SYLLABIC_OTHER = 0,
    DECODER_INDIC_SYLLABIC_AVAGRAHA,
    DECODER_INDIC_SYLLABIC_BINDU,
    DECODER_INDIC_SYLLABIC_BRAHMI_JOINING_NUMBER,
    DECODER_INDIC_SYLLABIC_CANTILLATION_MARK,
    DECODER_INDIC_SYLLABIC_CONSONANT,
    DECODER_INDIC_SYLLABIC_CONSONANT_DEAD,
    DECODER_INDIC_SYLLABIC_CONSONANT_FINAL,
    DECODER_INDIC_SYLLABIC_CONSONANT_HEAD_LETTER,
    DECODER_INDIC_SYLLABIC_CONSONANT_INITIAL_POSTFIXED,
    DECODER_INDIC_SYLLABIC_CONSONANT_KILLER,
    DECODER_INDIC_SYLLABIC_CONSONANT_MEDIAL,
    DECODER_INDIC_SYLLABIC_CONSONANT_PLACEHOLDER,
    DECODER_INDIC_SYLLABIC_CONSONANT_PRECEDING_REPHA,
    DECODER_INDIC_SYLLABIC_CONSONANT_PREFIXED,
    DECODER_INDIC_SYLLABIC_CONSONANT_SUBJOINED,
    DECODER_INDIC_SYLLABIC_CONSONANT_SUCCEEDING_REPHA,
    DECODER_INDIC_SYLLABIC_CONSONANT_WITH_STACKER,
    DECODER_INDIC_SYLLABIC_GEMINATION_MARK,
    DECODER_INDIC_SYLLABIC_INVISIBLE_STACKER,
    DECODER_INDIC_SYLLABIC_JOINER,
    DECODER_INDIC_SYLLABIC_MODIFYING_LETTER,
    DECODER_INDIC_SYLLABIC_NON_JOINER,
    DECODER_INDIC_SYLLABIC_NUKTA,
    DECODER_INDIC_SYLLABIC_NUMBER,
    DECODER_INDIC_SYLLABIC_NUMBER_JOINER,
    DECODER_INDIC_SYLLABIC_PURE_KILLER,
    DECODER_INDIC_SYLLABIC_REGISTER_SHIFTER,
    DECODER_INDIC_SYLLABIC_REORDERING_KILLER,
    DECODER_INDIC_SYLLABIC_SYLLABLE_MODIFIER,
    DECODER_INDIC_SYLLABIC_TONE_LETTER,
    DECODER_INDIC_SYLLABIC_TONE_MARK,
    DECODER_INDIC_SYLLABIC_VIRAMA,
    DECODER_INDIC_SYLLABIC_VISARGA,
    DECODER_INDIC_SYLLABIC_VOWEL,
    DECODER_INDIC_SYLLABIC_VOWEL_DEPENDENT,
    DECODER_INDIC_SYLLABIC_VOWEL_INDEPENDENT,
    DECODER_INDIC_SYLLABIC_COUNT
} decoder_indic_syllabic_category_t;

/**
 * @brief Indic_Positional_Category property (UAX #44).
 *
 * Sourced from `IndicPositionalCategory.txt`. The default value `NA` covers
 * code points that have no positional classification (most non-Indic scripts).
 */
typedef enum {
    DECODER_INDIC_POSITIONAL_NA = 0,
    DECODER_INDIC_POSITIONAL_RIGHT,
    DECODER_INDIC_POSITIONAL_LEFT,
    DECODER_INDIC_POSITIONAL_VISUAL_ORDER_LEFT,
    DECODER_INDIC_POSITIONAL_LEFT_AND_RIGHT,
    DECODER_INDIC_POSITIONAL_TOP,
    DECODER_INDIC_POSITIONAL_BOTTOM,
    DECODER_INDIC_POSITIONAL_TOP_AND_BOTTOM,
    DECODER_INDIC_POSITIONAL_TOP_AND_RIGHT,
    DECODER_INDIC_POSITIONAL_TOP_AND_LEFT,
    DECODER_INDIC_POSITIONAL_TOP_AND_LEFT_AND_RIGHT,
    DECODER_INDIC_POSITIONAL_BOTTOM_AND_RIGHT,
    DECODER_INDIC_POSITIONAL_BOTTOM_AND_LEFT,
    DECODER_INDIC_POSITIONAL_TOP_AND_BOTTOM_AND_RIGHT,
    DECODER_INDIC_POSITIONAL_TOP_AND_BOTTOM_AND_LEFT,
    DECODER_INDIC_POSITIONAL_OVERSTRUCK,
    DECODER_INDIC_POSITIONAL_COUNT
...
/**
 * @brief Conflict-resolution policy for the script unification engine.
 *
 * `STRICT` resolves overlaps by priority (lower priority value wins) with
 * `ucd_authoritative` as a tie-breaker. `LENIENT` ignores priority entirely
 * and lets the first-added range cover the overlapping region.
 */
typedef enum {
    DECODER_SCRIPT_RESOLUTION_STRICT = 0,   /**< Highest-priority range wins. */
    DECODER_SCRIPT_RESOLUTION_LENIENT       /**< First-added range wins. */
} decoder_script_resolution_mode_t;

/**
 * @brief Priority of a script range relative to others covering the same point.
 *
 * Lower numeric values bind more strongly: when two ranges overlap on a code
 * point, the one with the smaller priority value provides the resolved script.
 */
typedef enum {
    DECODER_SCRIPT_PRIORITY_PRIMARY   = 0,  /**< UCD-authoritative mapping. */
    DECODER_SCRIPT_PRIORITY_SECONDARY = 1,  /**< Alternate or split mapping. */
    DECODER_SCRIPT_PRIORITY_LEGACY    = 2   /**< Deprecated / fallback only. */
} decoder_script_priority_t;

/** Maximum length (including NUL) of script-label and source strings. */
#define DECODER_SCRIPT_LABEL_MAX 64

/**
 * @brief A single script range with priority metadata.
 *
 * Used both as input to the unification engine and as the gap-output type.
 * For gap entries only `start` and `end` are meaningful; the metadata fields
 * are zero-initialised.
 */
typedef struct {
    uint32_t start;
    uint32_t end;
    char script[DECODER_SCRIPT_LABEL_MAX];
    decoder_script_priority_t priority;
    bool ucd_authoritative;
    char source[DECODER_SCRIPT_LABEL_MAX];
} decoder_script_range_t;

/**
 * @brief Result of a single overlap detected by the resolver.
 *
 * One result is emitted per overlapping pair found. `resolved_script` carries
 * the script label of the higher-priority entry in that pair.
 */
typedef struct {
    uint32_t start;
    uint32_t end;
    char resolved_script[DECODER_SCRIPT_LABEL_MAX];
} decoder_script_conflict_result_t;

/**
 * @brief Mutable script unification engine.
 *
 * Holds a dynamic list of user-supplied script ranges. The struct fields are
 * deliberately public so tests and tooling can inspect engine state.
 */
typedef struct {
    decoder_script_range_t *ranges;
    size_t range_count;
    size_t range_capacity;
    decoder_script_resolution_mode_t mode;
    bool strict_mode;
