/**
 * @file entry.c — typecheck AST walk.
 */
    ...
    for (uint32_t i = 0; i < tu->child_count; i++) {
        ...
        if (n->kind == WGSL_NODE_DIR_ENABLE) {
            for (uint32_t j = 0; j < n->child_count; j++) {
                ...
                uint32_t nl  = wgsl_node_name_span(arg).length;
             ...
             * we silently accept). */
            uint32_t rule_parts = wgsl_diag_rule_parts(n);
    ...
    WGSLTypeChecker     *out)
{
    return wgsl_typecheck_with_opts(
        ast, src, arena, diag, types, res, cev, out, NULL);
}
    ...
    const WGSLTypecheckOpts *opts)
{
    ...
    if (opts && opts->skip_body_count > 0 &&
        opts->skip_body_names && opts->skip_body_name_lens)
    {
        out->skip_body_names     = opts->skip_body_names;
        out->skip_body_name_lens = opts->skip_body_name_lens;
        out->skip_body_count     = opts->skip_body_count;
    }

    if (!ast->root) return 0;
    WGSLNode *tu = ast->root;

    /* Refuse a pathologically deep AST up front — the type-checker's
     * recursive walkers would otherwise overflow the stack. */
    {
        const WGSLNode *deep = wgsl_ast_first_over_depth(tu, WGSL_MAX_AST_DEPTH);
        if (deep) {
            wgsl_tc_error(out, deep,
     ...
     * downstream type-specs that reference them resolve.  Recursive type
     * cycles are diagnosed by the validator. */
    for (uint32_t i = 0; i < tu->child_count; i++) {
        WGSLNode *n = tu->children[i];
        if (!n) continue;
