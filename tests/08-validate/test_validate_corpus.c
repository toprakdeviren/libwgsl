    ...
    const char *bytes, size_t len, const char *display, FileStats *st)
{
    ...
    int val_ok    = wgsl_validate(&ast, &src, &arena, &diag, &ts, &res, &cev, &tc, &val);

    int ok = lex_ok && parse_ok && res_ok && ce_ok && tc_ok && val_ok &&
             !wgsl_diag_has_error(&diag);
    if (ok) {
        st->sections_ok += 1;
    } else {
        fprintf(stderr,
            "FAIL  %s — lex=%d parse=%d res=%d ce=%d tc=%d val=%d  %zu diag(s):\n",
            display, lex_ok, parse_ok, res_ok, ce_ok, tc_ok, val_ok,
...
int main(void) {
    ...
    if (fail == 0) {
        printf("PASS  test_validate_corpus  %d sections\n", total_sections);
        return 0;
    }
    fprintf(stderr,
        "FAIL  test_validate_corpus  %d section(s) failed\n", fail);
    return 1;
}
